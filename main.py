...
# spaceship-ufo game by sanket and sagar
class Spaceship(Widget):
    def __init__(self, **kwargs):
        ...
        with self.canvas:
            Rectangle(source="background3.png", pos=(0, 0),
        ...
        Clock.schedule_interval(self.ufo, 2)
    ...
    def ufo(self, dt):
        for i in range(5):
            ...
            self.add_enemy(Enemy((random_x, y), random_speed))
...
class Player(Entity):
    ...
    def shoot_step(self, dt):
        # shoot
        if "spacebar" or "5" in game.keysPressed:
            x = self.pos[0]
            y = self.pos[1] + 50
            game.add_enemy(Bullet((x, y)))
    ...
    def move_step(self, sender, dt):
        ...
        newx = self.pos[0]
        newy = self.pos[1]
        if "4" in game.keysPressed:
            newx -= step_size
        if "6" in game.keysPressed:
            newx += step_size
        self.pos = (newx, newy)
    ...
    def move_step(self, sender, dt):
        ...
        for e in game.colliding_entities(self):
            if isinstance(e, Enemy):
                game.add_enemy(Explosion(self.pos))
                self.stop_callbacks()
                game.remove_enemy(self)
                e.stop_callbacks()
                game.remove_enemy(e)
...
game.add_enemy(game.player)

player2 = Player()
player2.pos = (Window.width / 3, 0)
game.add_enemy(player2)
